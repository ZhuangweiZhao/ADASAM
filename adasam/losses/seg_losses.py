...
用于语义分割掩码监督的二值损失, 集中定义。
Binary losses for semantic segmentation mask supervision, centralized here.
...
形状 [B, H, W] 或 [H, W] | shapes [B, H, W] or [H, W].
"""
...
def dice_loss(
    ...
    """Soft Dice loss | Soft Dice loss.

    对 [N, H, W]: 对 N 个 mask 独立计算 Dice, 再按 reduction 归约。
    For [N, H, W]: Dice per mask, then reduced across N.
    """
