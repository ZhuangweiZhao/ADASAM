...
from adasam.datasets import ISAIDInstanceDataset, DEFAULT_FOLDS
...
def test_sample_contract(ds_base):
    ...
    s = ds_base[0]
    assert set(s.keys()) == {"image", "instances", "tile_id", "orig_image_id", "tile_origin"}
    ...
    assert isinstance(s["instances"], list)
...
def test_instances_restricted_to_visible(ds_base):
    ...
    for i in range(min(len(ds_base), 50)):
        for inst in ds_base[i]["instances"]:
            assert inst["category_id"] in visible
...
def test_query_interface(ds_base):
    ...
    idx = tiles[0]
    cats = {inst["category_id"] for inst in ds_base[idx]["instances"]}
    assert cls in cats
