...
def test_forward_train_shapes(model):
    ...
    dpg_out, low_res, iou_pred = model.forward_train(emb, proto)
...
def test_forward_train_to_criterion_backward(model):
    """全链路: forward_train → criterion → backward, 梯度可达 DPG | full-chain gradients."""
    emb = torch.randn(1, 256, 64, 64)
    proto = F.normalize(torch.randn(256), dim=0)
...
def test_predict_contract(model):
    ...
    masks, scores = model.predict(
        emb, proto, input_size=(1024, 1024), original_size=(896, 896), score_thr=0.0
    )
    n = masks.shape[0]
...
def test_predict_high_threshold_returns_empty(model):
    """score_thr=1.1 → 空输出不报错 | impossible threshold → empty output, no crash."""
    emb = torch.randn(1, 256, 64, 64)
    proto = F.normalize(torch.randn(256), dim=0)
    masks, scores = model.predict(
        emb, proto, input_size=(1024, 1024), original_size=(896, 896), score_thr=1.1
    )
...
def test_state_dict_roundtrip(model):
    """state_dict 严格加载往返 | strict state_dict round-trip."""
    from adasam.backbone import build_mobile_sam
